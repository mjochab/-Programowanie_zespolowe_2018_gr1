...
public interface PatientListMapper {
    @Select("select s.id_admission_day, visit_hour, s.id_patient, p.first_name, p.last_name from singlevisits s inner join admissiondays as a JOIN patients p on s.id_patient=p.id_patient where a.id_admission_day = s.id_admission_day and date='2018-06-10'")
    @Results({
